...
/*   Updated: 2024/01/28 22:32:39 by klamprak         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// 7x7
// ./a.out "3 2 3 2 2 2 1 4 5 1 3 2 3 7 7 2 2 1 2 4 3 1 2 3 4 2 3 4"
...
// declarations of optimize.c
void	fill_board(int board[S][S], int h_ar[S]);
int		is_pattern(int row, int col, int guess, int h_ar[S]);
...
int	main(int argc, char *argv[])
{
	...
	fill_board(board, inp_heigh_arr);
...
int	solve(int board[S][S], int inp_heigh_arr[S4])
{
	...
	while (guess <= S)
	{
		...
		is_v = is_v && is_pattern(row, column, guess, inp_heigh_arr);
		if (is_v)
		{
